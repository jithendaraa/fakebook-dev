...
class Chatbox extends Component {
    ...
    componentDidMount() {
        ...
        this.props.socket.on("connect", () => {
            ...
            this.props.socket.on("output", async chats => {
                ...
                if(this.state.textObj !== 0){
                    console.log("tf" + this.state.textObj.message);

                    this.parentDivChild(this.state.textObj);
                    await this.setState({ textObj: 0 });
                }
            });
        ...
        this.props.socket.on('message', async textObj => {
            ...
            // await this.setState({ textObj: textObj });
            ...
            // this.parentDivChild(textObj);
            // if((textObj.fromId !== this.props.auth._id) && (textObj.fromId !== this.props.openChatId)){                                  //If message is not from myself
    ...
    setChat = () => {
        console.log("hmm");
        console.log(this.props.openChatId)
        if(this.props.openChatId !== null){
            console.log("im not null bitch")
            let openChatId = this.props.openChatId;
            let openChatName = this.props.openChatName;
