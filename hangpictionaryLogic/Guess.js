...
class Guess extends Component {
    ...
    state = {
        totalLetters: (this.props.word.length * 2),
        letters: [],
        wrongs: 0
    }
    ...
    async componentDidMount() {
        // let letterNos = this.state.totalLetters.length;
    ...
    renderDashes = () => {
        let i = 0;
        return this.props.word.split('').map(word => {
            i += 1;
    ...
    clicked = (id) => {
        ...
        let wordLetters = this.props.word.split('');
    ...
    render() {
        return (
            <div style={{backgroundColor: "rgb(15, 15 ,15)"}}>
                <h3 style={{ textAlign: "center", color: "rgb(173,255,27)" }}>Guess ({this.props.word !== undefined ? this.props.word.length : null}) totalletters{this.state.totalLetters}</h3>
